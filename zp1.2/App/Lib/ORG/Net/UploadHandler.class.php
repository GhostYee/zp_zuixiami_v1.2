...
class UploadHandler
{
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        // 重新设定保存规则
        preg_match('/\.\w+$/', $name, $save_type);
        $save_name = str_replace('.', '_', microtime(true).'_'.rand(1000,9999)).(!empty($save_type[0]) ? $save_type[0] : '');

        $file->name = $this->get_file_name($save_name, $type, $index, $content_range);
