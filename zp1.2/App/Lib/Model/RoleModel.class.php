...
function setGroupActions($groupId,$actionIdList)
{
    ...
    function delGroupAll($groupId)
{
    $table = $this->tablePrefix.'access';

    $result = $this->db->execute('delete from '.$table.' where level=3 and role_id='.$groupId);
    if($result===false) {
        return false;
    }else {
        return true;
...
{
    $table = $this->tablePrefix.'access';
    $rs = $this->db->query('select b.id,b.title,b.name from '.$table.' as a ,'.$this->tablePrefix.'node as b where a.node_id=b.id and  a.role_id='.$groupId.' ');
    return $rs;
}
